...
export EKS_AWS_ADMIN_ROLE="ed-k8s-us-east-1-Administrators"
export EKS_ADMIN_PROFILE="eks-admin"
export EKS_CLUSTER_NAME="ed-k8s"
export EKS_REGION="us-east-1"

echo "get the role arn for $EKS_AWS_ADMIN_ROLE"
EKS_AWS_ADMIN_ROLE_ARN=$(aws iam get-role --role-name $EKS_AWS_ADMIN_ROLE --output text --query 'Role.Arn')
echo
echo "Create assume role credentials file using: $EKS_AWS_ADMIN_ROLE arn"
 aws sts assume-role --role-arn $EKS_AWS_ADMIN_ROLE_ARN \
  --role-session-name "EKS-role-for-me" > assume-role-output.json 
echo
